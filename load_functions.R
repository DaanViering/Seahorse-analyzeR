## -------------Functions for Seahorse-analyzeR ---------------------
...
#Removes measurements where the OCR was 0 (i.e. background) 
#or lower than background.
remove_wells <- function(dt, method = "without transformation", 
     ...
     b <- a - nrow(dt2)
     print(paste0(b, " background measurements removed (", b/16, " wells)")) 
     #16 is number of measurement intervals
...
#Calculates the average of each interval 
#(distinguishing different FCCP concentrations) for each cell culture
...
pivot_ints_to_cols <- function(dt) {
  dt %>%
    ...
    summarise(median_int = median(median_int), 
              interval = first(interval), sample_id = first(sample_id)) %>%
    pivot_wider(id_cols = sample_id, 
                names_from = interval, values_from = median_int)
}
